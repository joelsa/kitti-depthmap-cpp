...
std::vector<float> Calibration::getValuesByKey(const std::string file, const std::string inKey) {
...
void Calibration::loadVelo2Cam(const std::string file) {
  ...
  std::vector<float> Rot_values = getValuesByKey(file, std::string("R")); // Rotation vector
  Eigen::Matrix3f Rot_v2c = Eigen::Map<Eigen::Matrix<float, 3, 3> >(Rot_values.data());
  Rot_v2c.transposeInPlace();

  std::vector<float> Transl_values = getValuesByKey(file, std::string("T")); // Translation Vector  
  Eigen::Vector3f Transl_v2c = Eigen::Map<Eigen::Matrix<float, 1, 3> >(Transl_values.data()); 
...
void Calibration::loadCam2Cam(const std::string file, const std::string camera) {
  
  std::vector<float> Proj_values = getValuesByKey(file, std::string("P_rect_") + std::string(camera)); // Rotation vector
