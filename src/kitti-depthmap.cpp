...
cv::Mat overimposeDepth(const cv::Mat &background, const cv::Mat &depthmap,
                        const int colorMap) {
...
std::vector<DepthRanging>
transformCloudToCamera(std::vector<Eigen::Vector3f> cloud, Calibration calib,
                       cv::Mat img) {
  ...
  for (Eigen::Vector3f lidar_pt : cloud) {
    ...
    if (pixel_pt(2) > 0) {
      ...
      DepthRanging ranging(range, (int)pixel_pt(0), (int)pixel_pt(1));
      if (ranging.fitsInto(img)) {
        rangings.push_back(ranging);
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  cv::Mat normalized =
      cv::Mat(input_img.rows, input_img.cols, CV_8UC3, cv::Scalar(0, 0, 0));
  cv::Mat raw =
      cv::Mat(input_img.rows, input_img.cols, CV_32FC1, cv::Scalar(0.0));

/* ######################################################################### */


  std::vector<DepthRanging> rangings =
      transformCloudToCamera(cloud, calib, input_img);

  std::vector<DepthRanging>::iterator min_point_it =
      std::min_element(rangings.begin(), rangings.end());
  std::vector<DepthRanging>::iterator max_point_it =
      std::max_element(rangings.begin(), rangings.end());
  double min = min_point_it->range;
  double max = max_point_it->range;
  int scale = 255;
  ...
  for (DepthRanging point : rangings) {
    ...
    cv::circle(normalized, position, radius, cv::Scalar(color, color, color),
               cv::FILLED, 8, 0);
  }

  cv::Mat output_img =
      overimposeDepth(input_img, normalized, cv::COLORMAP_VIRIDIS);
